...
def write_img_newfolder(dataset_dir, train_folder_index, test_folder_index, training_dir, testing_dir):
    ...
    for i in range(len(train_folder_index)):
        ...
        while os.path.exists(image_j):
            ...
            shutil.copyfile(image_j, image_new)
            j = j + 1
            image_j = img_dir + 'frame-' + ('%06d' % j) + '.color.png'
...
if __name__ == '__main__':
    ...
    train_folder_index, test_folder_index = parse_data_folder_index(
        train_txt, test_txt)

    #   II. Put training data in 00 and testing data in 01 folder
    training_dir = dataset_dir + '/sequences/train/'
    testing_dir = dataset_dir + '/sequences/test/'
    write_img_newfolder(dataset_dir, train_folder_index,
                        test_folder_index, training_dir, testing_dir)

    #   III. Put trajectory in the folder
    text_file_train = os.path.join(dataset_dir, 'train_rotation.txt')
    text_file_test = os.path.join(dataset_dir, 'test_rotation.txt')
    write_pose_newfile_rotation(
        text_file_train, text_file_test, train_folder_index, test_folder_index)
    text_file_train = os.path.join(dataset_dir, 'train_quaternion.txt')
    text_file_test = os.path.join(dataset_dir, 'test_quaternion.txt')
    write_pose_newfile_quaternion(
        text_file_train, text_file_test, train_folder_index, test_folder_index)
