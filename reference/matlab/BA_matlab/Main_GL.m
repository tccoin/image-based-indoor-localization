...
%         observe_ith{j}.robotpose = groundtruth.robotpose(locID,:);
        ...
        [ observe_ith{j},param{j}] = process_7scene_SIFT(history,cameraParams,observe_ith{j},filepath,filepath_history,num_image,gap,locID,locID_init,param_keyframe, i);
...
X_fig = 1:1:length(error_angle);

%%
figure(1);
plot(X_fig,error_angle);
xlabel('Time [# of images]')
ylabel('Vanilla GL absolute angle error [Degree]')
title('Absolute Angle Error (AAE) versus Time Index - Vanilla GL')
hold on;

figure(2);
plot(X_fig,error_dis);
xlabel('Time [# of images]')
ylabel('Vanilla GL absolute trajectory error [m]')
title('Absolute Trajectory Error (ATE) versus Time Index - Vanilla GL')
hold on;
